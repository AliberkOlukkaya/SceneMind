...
The offline [Hybrid Fusion Refinement V1](ml/evaluation/HYBRID_FUSION_REFINEMENT_V1.md) compares three capped-overlap and three normalized-rank formulas after proving exact baseline parity. A simple 1.50× shared-contribution cap improves development Top-5 from 20/28 to 21/28 and MRR@5 from 0.4827 to 0.5155 with one rescue, no newly broken query, and unchanged Visual/Multimodal Top-K counts. It remains a development candidate requiring new source-disjoint holdout validation. Runtime Hybrid retrieval is still the original uncapped RRF60 implementation.
