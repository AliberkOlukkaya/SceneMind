...
OCR Evidence V1 is an isolated evaluation branch in `backend/app/ocr_evidence.py`. It normalizes OCR text, suppresses adjacent near-duplicates, persists fingerprinted timestamp evidence and searches it with the existing BM25 primitive. It has no API route, worker hook or frontend surface. Frozen validation rejected integration after 84.14% frozen-label/84.84% audited character accuracy, 63.64% end-to-end Recall@5 and 2.304-second CPU p95 latency. Production frames are not OCR-indexed.
