"""Download and verify verifier calibration-development media."""
...
from ml.experiments.image_text_verifier.schema import load_calibration  # noqa: E402
...
def prepare(manifest: Path, verify_only: bool = False) -> None:
    dataset = load_calibration(manifest)
    for video in dataset.videos:
        ...
        if not destination.exists():
            ...
            request = urllib.request.Request(
                ...
                headers={"User-Agent": "SceneMindVerifierExperiment/1.0"},
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--manifest", type=Path, default=Path(__file__).with_name("calibration_v1.json")
    )
