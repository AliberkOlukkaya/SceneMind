...
const timestamp = (value: number) => `${Math.floor(value / 60)}:${Math.floor(value % 60).toString().padStart(2, "0")}`;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        ...
        if (active) { setVideos(data); setLoading(false); }
      } catch { if (active) { setError("Cannot reach the video service. Check that the backend is running."); setLoading(false); } }
    }
    void refresh();
  ...
  async function upload(file?: File) {
    if (!file) return;
    setError("");
    if (file.size > 250 * 1024 * 1024) { setError("Choose a video smaller than 250 MB."); return; }
    setUploading(true);
    try {
      ...
      setVideos(previous => [data, ...previous.filter(item => item.id !== data.id)]);
  ...
  return <main>
    <header><Link className="brand" href="/">SceneMind<span> / workspace</span></Link><span className="status">Local library</span></header>
    <section className="heading toolbar"><div><p className="eyebrow">YOUR WORKSPACE</p><h1>Video library</h1><p>{videos.length} videos · Browse sampled moments</p></div>
      <div><input ref={input} type="file" accept=".mp4,.mov,.webm,.mkv,.avi" aria-label="Choose video" className="sr-only" onChange={event => void upload(event.target.files?.[0])} /><button className="button" disabled={uploading} onClick={() => input.current?.click()}>{uploading ? "Uploading…" : "+ Upload video"}</button><p className="hint">Up to 250 MB · 30 minutes · 4K</p></div>
    </section>
    {error && <p role="alert" className="error">{error} <button onClick={() => setError("")}>Dismiss</button></p>}
    {loading ? <section className="empty" aria-live="polite">Loading your library…</section> : videos.length === 0 ? <section className="empty"><span className="film" aria-hidden="true">?</span><h2>Add your first video.</h2><p>Upload a video to extract frames and browse its timeline.</p><button className="button" disabled={uploading} onClick={() => input.current?.click()}>Choose a video</button></section> :
