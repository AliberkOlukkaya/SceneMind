...
type Result = { timestamp: number; thumbnail: string; score: number; modality: string; text?: string };

export default function Search({ videoId, api, seek }: {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        ...
        if (active) { setStatus(data.status); if (data.error) setError(data.error); }
      } catch (problem) { if (active) setError(problem instanceof Error ? problem.message : "Index unavailable."); }
    }
    void refresh();
  ...
  async function index() {
    ...
    try {
      const response = await fetch(`${api}/videos/${videoId}/index`, { method: "POST" });
      const data = await response.json();
      if (!response.ok) throw new Error(typeof data.detail === "string" ? data.detail : "Indexing failed.");
      setStatus("processing");
    } catch (problem) { setError(problem instanceof Error ? problem.message : "Indexing failed."); }
