...
Phases 0-6 and the bounded local Phase 10 follow-up are implemented, including Natural Video Benchmark V2. Phase 7 experiments rejected pair verifiers, global no-match, detector routing, bounded secondary sampling, cheap diversity and candidate-list scoring. A source-disjoint query-routing calibration now promotes a tiny text-only AUTO selector that matches explicit-route Oracle R@5 while retaining every manual mode. The next milestone is a private personal-video acceptance run on real 30–60 minute material. Phases 8-9 stay deferred until that evidence identifies a concrete gap.
